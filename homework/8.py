...
class Fractal:
    ...
    def calculate_epochs(self, n):
        ...
        parse_exps = dict(parse_exps)

        # print(parse_exps)
        result = list(self.init)
        for _ in range(n):
            temp_list = []
            for i, elem in enumerate(result):
                if elem in parse_exps:
                    temp_list.extend(parse_exps[elem])
                else:
                    temp_list.append(elem)
            result, temp_list = temp_list, result
...
class DrawFractal(QWidget):
    ...
    def draw_fractal(self, qp):
        ...
        fractal = self.fractal.calculate_epochs(10)
        print(fractal)
        angle = 0
        dump = None
        for sym in fractal:
            if sym == 'f' or sym == "F":
                ...
                dump = [current_x, current_y, angle]
            elif sym == "]":
                current_x, current_y, angle = dump
            elif sym == "|":
                angle += 180
